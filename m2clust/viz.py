...
from . import config, distance, dataprocess
...
def dendrogram_plot(data_table, D=[], xlabels_order = [], xlabels = None, ylabels = [], 
            filename=config.output_dir+'/m2clust_dendrogram', metric = config.similarity_method, linkage_method = "single",
...
def ord_plot(coords, target_names = None, ord_name = 'ord',\
    ...
    for target_name in sorted_key_by_len_large:
        label_flag =  True
        if metadata is not None:
            point_mrakers = [markers_dic[val] for val in map(str, metadata.ix[target_names_large[target_name],str(shapeby)])]
        else:
    ...
    for target_name in target_names_small:
        if metadata is not None and shapeby is not None:
            point_mrakers = [markers_dic[val] for val in map(str, metadata.ix[target_names_small[target_name],str(shapeby)])]
        else:
    ...
    for i, val in enumerate( set(metadata[shapeby])):
        ...
        ax.scatter(0, 
                   ...
                   s =10, alpha = .1, linewidths = .25, edgecolors = 'black',
                   label= lablel)'''
    ...
    ax.legend([handle for i,handle in enumerate(handles) if i in range(to_display) ]+ markerArtist[0:to_display_markers],
              [label for i,label in enumerate(labels) if i in range(to_display)]+ order_metadata[0:to_display_markers],
              loc='lower right', title = 'Cluster: size', shadow=False, scatterpoints=1, 
...
def main( ):
    ...
    mds_ord(df_distance, target_names = clusters, size_tobe_colered = args.size_to_plot, metadata = metadata, shapeby = args.shapeby)
    
    pcoa_ord(df_distance, target_names = clusters, size_tobe_colered = args.size_to_plot, metadata = metadata, shapeby = args.shapeby)
    
    tsne_ord(df_distance, target_names = clusters, size_tobe_colered = args.size_to_plot, metadata = metadata, shapeby = args.shapeby)
    #if data_flag:
